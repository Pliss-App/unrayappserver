...
const updateUser = (nombre, apellido, telefono, correo, id) => { //getByEmail
    return new Promise((resolve, reject) => {
        connection.query(
            "UPDATE usuario SET nombre= ?, apellido=?, correo=? WHERE id=? ;", [nombre.toUpperCase(), apellido.toUpperCase(), correo.toUpperCase(), id], (err, rows) => {
                if (err) reject(err)
                resolve(rows)
            });
    });
...
const insertTiempoUsuarioDiario = (idUser, fecha, duracion) => {
    return new Promise((resolve, reject) => {
        const query = `
      INSERT INTO tiempoUsoDiario (idUser, fecha, duracionsegundos)
      VALUES (?, ?, ?)
...
const puedeEnviarSMS = (numero) => {
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql, [numero], (err, rows) => {
      if (err) return reject(err);
      resolve(rows[0].total < 7); // máximo 3 envíos por día
    });
  });
};
