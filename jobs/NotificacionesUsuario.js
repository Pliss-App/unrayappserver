...
cron.schedule('* * * * *', async () => {
  ...
  const notificaciones = await isNoti.getNotificaciones('usuario');
  for (const noti of notificaciones) {
    ...
    //console.log(`[🔎] Noti ID Usuario ${id} | ahora: ${ahora.toFormat('HH:mm:ss')} | próximo: ${proximoEnvio.toFormat('HH:mm:ss')} | diferencia: ${diferencia}`);
